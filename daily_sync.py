...
Samodzielny, skonsolidowany program do codziennej synchronizacji ofert OLX
(pokoje/stancje, Warszawa) z lokalną bazą danych SQLite (`data.db`).

Zawiera w jednym pliku całą logikę, która wcześniej była podzielona na
main.py (scraping listingu + opisów), room_analysis.py (analiza opisu przez
lokalny model AI w Ollamie) i fetch_photos.py (pobieranie zdjęć z galerii
ogłoszenia).

Działanie:
- Program uruchamia się jako proces stały (daemon) i co noc o godzinie 2:00
...
Uruchomienie:
    python daily_sync.py            # tryb ciągły - czeka do 2:00 każdej nocy
    python daily_sync.py --once     # wykonuje jedną synchronizację od razu i kończy
"""
...
DB_FILE = "data.db"
LEGACY_CSV_FILE = "data.csv"  # do jednorazowej migracji, jeśli baza jeszcze nie istnieje
...
def run_forever() -> None:
    logger.info("Uruchomiono daily_sync w trybie ciągłym (godzina uruchamiania: %02d:%02d).", RUN_HOUR, RUN_MINUTE)
    while True:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Codzienna synchronizacja ofert OLX z bazą SQLite (data.db)")
    parser.add_argument(
        "--once",
        action="store_true",
        help="Wykonaj jedną synchronizację natychmiast i zakończ (bez czekania do 2:00).",
    )
